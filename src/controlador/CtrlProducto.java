...
public class CtrlProducto extends CtrlImprimir implements ActionListener, CaretListener, MouseListener, KeyListener {
	...
	public CtrlProducto(Productos p, IMenu menu, int permiso) {
		...
		this.menu.mnMoverProducto.addActionListener(this);
		this.menu.btnMoverProducto.addActionListener(this);
		this.id = null;
	...
	public void iniciar() {
		...
		this.txtAnotacion = new JTextField();
	}
	...
	public void actionPerformed(ActionEvent e) {
		...
		}else if(e.getSource() == this.menu.mnMoverProducto){
			this.showWindowsMoverProducto();
		}else if(e.getSource() == this.menu.btnMoverProducto){
			this.moverProducto();
		}
	...
	public void showWindowsMoverProducto(){
		int filaseleccionada = this.menu.tblProductos.getSelectedRow();
		if(filaseleccionada != -1){
			this.id = (String) this.menu.tblProductos.getValueAt(filaseleccionada, 0);
			this.nombre = (String) this.menu.tblProductos.getValueAt(filaseleccionada, 2);
			this.menu.jdMoverProducto.setSize(415, 250);
			this.menu.jdMoverProducto.setLocationRelativeTo(null);
			this.menu.jdMoverProducto.setVisible(true);
		}
	}

	public void moverProducto(){
		this.stock = Float.parseFloat(this.menu.jsCantidadMoverProducto.getValue().toString());
		this.descripcion = this.menu.txtAreaAnotacionMoverProducto.getText();
		this.productos.guardarKardexIncial(
			Integer.parseInt(this.id),
			this.menu.lblUsuarioSistema.getText(),
			this.stock,
			"Salida de stock",
			this.descripcion
		);
		this.productos.Vender(this.id, String.valueOf(this.stock));
		this.menu.jdMoverProducto.setVisible(false);
...
//este metodo iversion es el mismo metodo que esta en reportes lo repite para no crear una nueva instancia de la clase reportes
	...
	public boolean validar() {
		...
		this.codigoBarra = menu.txtCodBarraProducto.getText();
		this.nombre = menu.txtNombreProducto.getText();
		this.precioCompra = Float.parseFloat(menu.spPrecioCompra.getValue().toString());
		this.precioVenta = Float.parseFloat(menu.spPrecioVenta.getValue().toString());
		float ganancia = (menu.txtGananciaProducto.getText().equals("")) ? 0 : Float.parseFloat(menu.txtGananciaProducto.getText());
		this.stock = Float.parseFloat(menu.txtCantidadProducto.getValue().toString());
		this.categoria = (menu.txtCategoriaProducto.getText().equals("")) ? 1 : Integer.parseInt(menu.txtCategoriaProducto.getText());
		this.laboratorio = (menu.txtLaboratorioProducto.getText().equals("")) ? 1 : Integer.parseInt(menu.txtLaboratorioProducto.getText());
		this.ubicacion = menu.txtUbicacionProducto.getText();
		this.descripcion = menu.txtDescripcionProducto.getText();
		this.monedaCompra = menu.cmbMonedaCompraProducto.getSelectedItem().toString();
		this.monedaVenta = menu.cmbMonedaVentaProducto.getSelectedItem().toString();
		this.precioMinimoVenta = Float.parseFloat(menu.spPrecioMinimoProducto.getValue().toString());
		this.utilidad = Float.parseFloat(menu.spUtilidad.getValue().toString());
		Date fecha = menu.jcFechaVProducto.getDate();
		long fechaV = fecha.getTime();
		this.fechaVencimiento = new java.sql.Date(fechaV);
		if (nombre.equals("")) {
			validar = false;
		} else {
			validar = true;
	...
	public void guardarProducto() {
		if (this.validar()) {
			...
			this.productos.setMonedaVenta(monedaVenta);
	...
	public void Kardex(String id) {
		this.productos.setId(Integer.parseInt(id));
		...
		this.menu.tblOtrasSalidasKardex.setModel(this.productos.kardexOtrasSalidas());
